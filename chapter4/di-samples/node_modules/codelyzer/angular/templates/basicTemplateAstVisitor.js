...
var getExpressionDisplacement = function (binding) {
    ...
    if (binding instanceof ast.BoundEventAst ||
        ...
        if (binding instanceof ast.BoundEventAst) {
            ...
            var content = binding.sourceSpan.end.file.content;
            var ngForSubstr = content.substring(binding.sourceSpan.start.offset, binding.sourceSpan.end.offset);
            result = ngForSubstr.lastIndexOf(binding.value.source) + '*ngFor'.length;
        }
        else {
